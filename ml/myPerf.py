...
def train_model(x):
    clsters = np.arange(2, 11)
    scores = []
    models = []
    for cluster in clsters:
        model = sc.KMeans(init='k-means++', n_clusters=cluster, n_init=10)
        model.fit(x)
        score = sm.silhouette_score(x, model.labels_, metric='euclidean', sample_size=len(x))
        scores.append(score)
        models.append(model)
    best_index = np.argmax(scores)
    best_clster = clsters[best_index]
    best_score = scores[best_index]
    best_model = models[best_index]
    print(best_clster, best_score)
    return best_model
